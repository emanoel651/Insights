...
def load_plenum() -> pd.DataFrame:
    df = pd.read_excel("meu_projeto\Plenum_2024-2025_ordenado.xlsx")
...
df = load_plenum()
if df.empty:
    st.error("Erro ao carregar os dados da base Plenum.")
    st.stop()
...
fig1 = px.line(
    stats_time,
    x="Emissão", y="Quantidade", color="Status",
    markers=True,
